...
class SegmentationTrainer:
    ...
    def __init__(self, model: nn.Module, train_loader: DataLoader, val_loader: DataLoader,
        ...
        self.instance_seg = hasattr(self.model, 'instance_seg') and self.model.instance_seg
    ...
    def train_epoch(self) -> float:
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            if self.instance_seg:
                binary_targets = batch['binary_mask'].to(self.device, non_blocking=True)
                hv_targets = batch['hv_map'].to(self.device, non_blocking=True)
            ...
            if self.use_amp:
                with autocast():
                    ...
                    if self.instance_seg:
                        loss, _ = self.criterion(outputs, labels.long(), binary_targets, hv_targets)
                    else:
                        loss = self.criterion(outputs['out'], labels.long())
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(pbar):
                ...
                if self.use_amp:
                    with autocast():
                        outputs = self.model(images)
                        predictions = outputs['out'] if isinstance(outputs, dict) else outputs
                        if self.instance_seg:
                            loss, _ = self.criterion(outputs, labels.long(), binary_targets, hv_targets)
                        else:
    ...
    def _extract_lora_and_decoder_head_params(self) -> Dict[str, torch.Tensor]:
        ...
        for name, tensor in self.model.state_dict().items():
            ...
            if ('lora_a' in name or 'lora_b' in name or
                name.startswith('decoder.') or name.startswith('segmentation_head.') or
                name.startswith('binary_head.') or name.startswith('hv_head.')):
    ...
    def _extract_decoder_head_params(self) -> Dict[str, torch.Tensor]:
        ...
        for name, tensor in self.model.state_dict().items():
            ...
            if (name.startswith('decoder.') or name.startswith('segmentation_head.') or
                name.startswith('binary_head.') or name.startswith('hv_head.')):
    ...
    def _extract_cnn_adapter_params(self) -> Dict[str, torch.Tensor]:
        ...
        for name, tensor in self.model.state_dict().items():
            ...
            if (name.startswith('cnn_adapter.') or name.startswith('decoder.') or
    ...
    def _extract_transformer_adapter_params(self) -> Dict[str, torch.Tensor]:
        ...
        for name, tensor in self.model.state_dict().items():
            ...
            if (name.startswith('transformer_adapter.') or name.startswith('decoder.') or
                name.startswith('segmentation_head.') or
                name.startswith('binary_head.') or name.startswith('hv_head.')):
